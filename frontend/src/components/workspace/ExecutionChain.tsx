...
type ExecutionChainProps = {
  label: string
  note: string
  steps?: { n: number; label: string; status: 'active' | 'pending' | 'done' }[]
}
...
export function ExecutionChain({ label, note, steps }: ExecutionChainProps) {
  const usedSteps = steps ?? defaultSteps
  return (
    <div>
      <div className="mb-4 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.2em] text-white/25">
        <span>Execution chain · live</span>
      ...
      <div className="flex items-center gap-3">
        ...
        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            ...
            exit={{ opacity: 0, y: -6 }}
            transition={{ duration: 0.3 }}
            className="ml-4 font-mono text-xs text-white/40"
          >
            {note}
